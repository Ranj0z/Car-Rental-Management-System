...
} from "../../src/All_Tables/reservation/reservation.service";

import { ReservationTable } from "../../src/Drizzle/schemas";
import db from "../../src/Drizzle/db";

jest.mock("../../../src/drizzle/", () => ({
  insert: jest.fn(),
  select: jest.fn(),
  update: jest.fn(),
  delete: jest.fn(),
...
describe("Reservation Service", () => {
  ...
  describe("getReservationByIdService", () => {
    it("should return a reservation by ID", async () => {
      ...
      const result = await getReservationByReservationIDService(1);
      expect(result).toEqual(reservation);
    });
  });
  ...
  describe("getReservationsByCustomerIdService", () => {
    it("should return reservations for a given customer ID", async () => {
      ...
      const result = await getReservationByCustomerIDService(4);
      expect(result).toEqual(reservations);
    });
  });
